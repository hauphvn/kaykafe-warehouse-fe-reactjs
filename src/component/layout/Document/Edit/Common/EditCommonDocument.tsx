import 'src/component/layout/Document/Edit/Common/EditCommonDocument.scss';
import { Controller, useForm } from 'react-hook-form';
import { yupResolver } from '@hookform/resolvers/yup/dist/yup';
import { Button, DatePickers, Input, MultipleSelect, Select } from 'src/component/common';
...
import {
  editCommonDocumentDefaultValues,
  editCommonDocumentSchema,
  endDateDefault,
  startDateDefault,
} from 'src/component/layout/Document/Edit/Common/EditCommonDocumentSchema';
...
const EditCommonDocument = (props: EditCommonDocumentPropsT) => {
  ...
  const {
    ...
    resolver: yupResolver(editCommonDocumentSchema()),
    mode: 'all',
    defaultValues: editCommonDocumentDefaultValues,
  });
  ...
  function onSubmit() {
    const dataUpdate: any = {
      // public_date: moment(getValues('periodDate').startDate).format(FORMAT_DATE_TIME.DATE_HOUR_00),
      // expired_date: moment(getValues('periodDate').endDate).format(FORMAT_DATE_TIME.DATE_FULL_SLASH),
      public_date: convertDateStart(getValues('periodDate').startDate),
      expired_date: convertDateEnd(getValues('periodDate').endDate),
      group_ids: getValues().groupIds,
  ...
  return (
    <ModalCustom
      className={'modal-edit-common-document'}
      ...
      onCancel={onCancel}
      title={'書類（共通）編集'}
      isShow={dataItemEdit?.showModalCommon ? dataItemEdit?.showModalCommon : false}
      okText={`編集`}
      cancelText={'キャンセル'}
      onSubmit={onSubmit}
      isDisable={!(isDirty && isValid)}
    >
      <div className={`edit-common-document-root`}>
        <div className="form">
          <Controller
            control={control}
            name="documentType"
          ...
          <Controller
            ...
            render={({ field: { onChange, onBlur, value } }) => (
              <MultipleSelect
                disabled={objDisabled.groupOption}
                options={groupOptions}
                onChange={onChange}
                required={true}
                label={'グループ '}
                className="text-item"
                textWarning={errors?.groupIds?.message ? errors?.groupIds?.message + '' : ''}
                onBlur={onBlur}
                value={value}
              />
            )}
      ...
      <input
        ...
        ref={refUploadFile}
        id={'upload-files-edit-common-document-id'}
        style={{ display: 'none' }}
        type="file"
